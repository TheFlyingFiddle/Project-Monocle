...
namespace Monocle.Graphics
{
    public struct Color 
    {
        ...
        public float R
        {
            get
            {
                return (packedValue & 0xFF ) / (float)0xFF;
            }
            set
            {
                packedValue = (packedValue & 0xFFFFFF00) | (uint)(value * 0xFF);
            }
        }
        ...
        public float G
        {
            get
            {
                return ((packedValue >> 8) & 0xFF) / (float)0xFF;
            }
            set
            {
                packedValue = (packedValue & 0xFFFF00FF) | ((uint)(value * 0xFF) << 8);
            }
        }
        ...
        public float B
        {
            get
            {
                return ((packedValue >> 16) & 0xFF) / (float)0xFF;
            }
            set
            {
                packedValue = (packedValue & 0xFF00FFFF) | ((uint)(value * 0xFF) << 16);
            }
        }
        ...
        public float A
        {
            get
            {
                return ((packedValue >> 24) & 0xFF) / (float)0xFF;
            }
            set
            {
                packedValue = (packedValue & 0x00FFFFFF) | ((uint)(value * 0xFF) << 24);
            }
        }

        public Color(uint hexColor)
        {
        ...
        {
            uint red = (uint)(Clamp(r) * 0xFF) << 0;
            uint green = (uint)(Clamp(g) * 0xFF) << 8;
            uint blue = (uint)(Clamp(b) * 0xFF) << 16;
            uint alpha = ((uint)(Clamp(a) * 0xFF) << 24);
        ...
        {
            scale = Clamp(scale);
            return new Color(color.R * scale, color.G * scale, color.B * scale, scale);
        }
        ...
        public override string ToString()
        {
            return string.Format("{ R:{0},G:{1},B:{2},A:{3} }",
                    (uint)(0xFF & this.packedValue),
                    (uint)(0xFF00 & this.packedValue) >> 8,
                    (uint)(0xFF0000 & this.packedValue) >> 16,
                    (uint)(0xFF000000 & this.packedValue) >> 24);
        }
